...
def mamba_mimo_fwd(
    ...
    def mamba_mimo_fwd_kernel(
        ...
        with T.Kernel(H, B, is_npu=True) as (i_h, i_b, _):
            # --- Kernel Setup ---
            ...
            v_shared = T.alloc_shared([chunk_size, 1, P], dtype)
            ...
            qk_dot_full_shared = T.alloc_shared([fused_chunk_size, fused_chunk_size], accum_dtype)
            ...
            for i in T.Pipelined(0, nchunks, num_stages=num_stages):
                ...
                # 这里parallel还不支持
                # for cs in T.serial(chunk_size): NOTE
                ...
                # for cs in T.Parallel(chunk_size): NOTE
                ...
                # for cs, r, n in T.Parallel(chunk_size, R, N): NOTE
                #     q_frag[cs, r, n] += q_bias_frag[r, n]
                for cs in T.serial(chunk_size):
                    T.vadd(q_frag[cs,:,:], q_bias_frag[0,:,:], q_frag[cs,:,:])
                ...
                for cs in T.serial(chunk_size):
                    T.vadd(k_frag[cs,:,:], k_bias_frag[0,:,:], k_frag[cs,:,:])
                ...
                q_first_half_frag = T.alloc_fragment([chunk_size, R, N//rotary_dim_divisor], accum_dtype)
                q_second_half_frag = T.alloc_fragment([chunk_size, R, N//rotary_dim_divisor], accum_dtype)


                # for cs, r, n in T.Parallel(chunk_size, R, N//rotary_dim_divisor): NOTE
                #     q_first_half_frag[cs, r, n] = q_shared[cs*R + r, n]
                #     q_second_half_frag[cs, r, n] = q_shared[cs*R + r, N//2 + n]
                for cs in T.serial(chunk_size):
                    offset = cs*R
                    T.copy(q_shared[offset:offset + R, : N//rotary_dim_divisor], q_first_half_frag[cs, :, :])
                    T.copy(q_shared[offset:offset + R, N//2 : N//2 + N//rotary_dim_divisor], q_second_half_frag[cs, :, :])

                # NOTE: angles are casted to fp32 for numerical stability
                angles_frag = T.alloc_fragment([1, chunk_size, N//rotary_dim_divisor], "float32")
                T.copy(ANGLES[i_b, chunk_start:chunk_start+chunk_size, i_h, :], angles_frag)
                angles_frag_cos = T.alloc_fragment([1, chunk_size, N//rotary_dim_divisor], "float32")
                T.vcos(angles_frag, angles_frag_cos)
                angles_frag_sin = T.alloc_fragment([1, chunk_size, N//rotary_dim_divisor], "float32")
                ...
                T.reshape(sincos_tmp, sincos_2d)
                q_shared_tmp = T.alloc_shared([1, R, N//rotary_dim_divisor], "float32")
                # for cs, r, n in T.Parallel(chunk_size, R, N//rotary_dim_divisor): NOTE
                #     q_shared[cs*R + r, n] = angles_frag_cos[cs, n] * q_first_half_frag[cs, r, n]
                #     q_shared[cs*R + r, n] -= angles_frag_sin[cs, n] * q_second_half_frag[cs, r, n]
                # for cs, r, n in T.Parallel(chunk_size, R, N//rotary_dim_divisor):
                #     q_shared[cs*R + r, N//2 + n] = angles_frag_sin[cs, n] * q_first_half_frag[cs, r, n]
                #     q_shared[cs*R + r, N//2 + n] += angles_frag_cos[cs, n] * q_second_half_frag[cs, r, n]
                for cs in T.serial(chunk_size):
                    offset = cs*R
                    T.vmul(angles_frag_cos[0, cs, :], q_first_half_frag[cs, :, :], 
                           q_shared_tmp)
                    T.copy(q_shared_tmp, q_shared[offset:offset + R, :N//rotary_dim_divisor])
                    T.vmul(angles_frag_sin[0, cs, :], q_second_half_frag[cs, :, :], sincos_tmp)
                    T.vsub(q_shared[offset:offset + R, :N//rotary_dim_divisor], sincos_2d, 
                           q_shared[offset:offset + R, :N//rotary_dim_divisor])

                    T.vmul(angles_frag_sin[0, cs, :], q_first_half_frag[cs, :, :],
                           q_shared_tmp) 
                    T.copy(q_shared_tmp, q_shared[offset:offset + R, N//2:N//2 + N//rotary_dim_divisor])
                    T.vmul(angles_frag_cos[0, cs, :], q_second_half_frag[cs, :, :], sincos_tmp)
                    T.vadd(q_shared[offset:offset + R, N//2:N//2 + N//rotary_dim_divisor], sincos_2d,
                           q_shared[offset:offset + R, N//2:N//2 + N//rotary_dim_divisor])
                ...
                k_first_half_frag = T.alloc_fragment([chunk_size, R, N//rotary_dim_divisor], accum_dtype)
                k_second_half_frag = T.alloc_fragment([chunk_size, R, N//rotary_dim_divisor], accum_dtype)
                ...
                for cs in T.serial(chunk_size):
                    offset = cs*R
                    T.copy(k_shared[offset:offset + R, : N//rotary_dim_divisor], k_first_half_frag[cs, :, :])
                    T.copy(k_shared[offset:offset + R, N//2 : N//2 + N//rotary_dim_divisor], k_second_half_frag[cs, :, :])

                # for cs, r, n in T.Parallel(chunk_size, R, N//rotary_dim_divisor): NOTE
                #     k_shared[cs*R + r, n] = angles_frag_cos[cs, n] * k_first_half_frag[cs, r, n]
                #     k_shared[cs*R + r, n] -= angles_frag_sin[cs, n] * k_second_half_frag[cs, r, n]
                # for cs, r, n in T.Parallel(chunk_size, R, N//rotary_dim_divisor):
                #     k_shared[cs*R + r, N//2 + n] = angles_frag_sin[cs, n] * k_first_half_frag[cs, r, n]
                #     k_shared[cs*R + r, N//2 + n] += angles_frag_cos[cs, n] * k_second_half_frag[cs, r, n]
                k_shared_tmp = T.alloc_shared([1, R, N//rotary_dim_divisor], accum_dtype)
                for cs in T.serial(chunk_size):
                    offset = cs*R
                    T.vmul(angles_frag_cos[0, cs, :], k_first_half_frag[cs, :, :], k_shared_tmp)
                    T.copy(k_shared_tmp, k_shared[offset:offset + R, :N//rotary_dim_divisor])
                    T.vmul(angles_frag_sin[0, cs, :], k_second_half_frag[cs, :, :], sincos_tmp)
                    T.vsub(k_shared[offset:offset + R, :N//rotary_dim_divisor], sincos_2d, k_shared[offset:offset + R, :N//rotary_dim_divisor])

                    T.vmul(angles_frag_sin[0, cs, :], k_first_half_frag[cs, :, :], k_shared_tmp)
                    T.copy(k_shared_tmp, k_shared[offset:offset + R, N//2:N//2 + N//rotary_dim_divisor])
                    T.vmul(angles_frag_cos[0, cs, :], k_second_half_frag[cs, :, :], sincos_tmp)
                    T.vadd(k_shared[offset:offset + R, N//2:N//2 + N//rotary_dim_divisor], sincos_2d,
                           k_shared[offset:offset + R, N//2:N//2 + N//rotary_dim_divisor])
                ...
                k_trap_scaled_frag = T.alloc_fragment([fused_chunk_size, N], accum_dtype)
                T.copy(k_shared, k_trap_scaled_frag)
                # for csr, n in T.Parallel(fused_chunk_size, N): NOTE
                #     k_trap_scaled_frag[csr, n] *= trap_scale_shared[csr//R]
                for cs in T.serial(chunk_size):
                    T.vmul(k_trap_scaled_frag[cs*R:cs*R+R, :], trap_scale_shared[cs], k_trap_scaled_frag[cs*R:cs*R+R, :])
                T.copy(k_trap_scaled_frag, k_shared)

                qk_intrachunk_frag = T.alloc_fragment([fused_chunk_size, fused_chunk_size], accum_dtype)
                ...
                qk_intrachunk_masked_frag = T.alloc_fragment([fused_chunk_size, fused_chunk_size], accum_dtype)
                T.clear(qk_intrachunk_masked_frag)
                T.vexp(segsum, segsum)
                # for csr_i, csr_j in T.Parallel(fused_chunk_size, fused_chunk_size): NOTE
                #     qk_intrachunk_masked_frag[csr_i, csr_j] = T.if_then_else(
                #                                 csr_i//R > csr_j//R, # we do indeed want to exclude the diagonal
                ...
                for cs_i in T.serial(chunk_size):
                    for cs_j in T.serial(cs_i):
                        T.vmul(qk_intrachunk_frag[cs_i*R:cs_i*R+R, cs_j*R:cs_j*R+R], segsum[cs_i, cs_j],
                                qk_intrachunk_masked_frag[cs_i*R:cs_i*R+R, cs_j*R:cs_j*R+R])
                ...
                # for csr, p in T.Parallel(fused_chunk_size, P): NOTE
                #     o_mimo_accum_frag[csr, p] *= exp_da_cs_frag[csr//R]
                for cs in T.serial(chunk_size):
                    T.vmul(o_mimo_accum_frag[cs*R:cs*R+R, :], exp_da_cs_frag[cs], o_mimo_accum_frag[cs*R:cs*R+R, :])
                # T.vcast(qk_intrachunk_masked_cast, qk_intrachunk_shared)
                ...
                # for cs, r_out, p in T.Parallel(chunk_size, R, P): NOTE
                ...
                qk_dot_tmp = T.alloc_shared([R, R], dtype)
                PsiV_tmp = T.alloc_shared([R, P], dtype)
                kv_dot_tmp = T.alloc_shared([R, P], accum_dtype)
                for cs in T.serial(chunk_size):
                    T.copy(qk_dot_full_shared[cs*R:cs*R+R,cs*R:cs*R+R], qk_dot_tmp)
                    ...
                    T.vmul(qkdot_psiv_frag[cs, :, :], gamma_frag[cs], qkdot_psiv_frag[cs, :, :])
                ...
                if hasD:
                    ...
                    for cs in T.serial(chunk_size):
                        T.copy(PsiV_shared[cs*R:cs*R+R, :], PsiV_D_frag[cs, :, :])
                    # for cs, r_out, p in T.Parallel(chunk_size, R, P): NOTE
                    #     qkdot_psiv_frag[cs, r_out, p] += d_i_h * PsiV_D_frag[cs, r_out, p]
                    T.vmul(PsiV_D_frag, d_i_h, PsiV_D_frag)
                    T.vadd(qkdot_psiv_frag, PsiV_D_frag, qkdot_psiv_frag)

                qkdot_psiv_reshaped_frag = T.alloc_fragment([fused_chunk_size, P], accum_dtype)
                T.reshape(qkdot_psiv_frag, qkdot_psiv_reshaped_frag)
                # for csr, p in T.Parallel(fused_chunk_size, P): NOTE
                #     o_mimo_accum_frag[csr, p] += qkdot_psiv_reshaped_frag[csr, p]
                T.vadd(qkdot_psiv_reshaped_frag, o_mimo_accum_frag, o_mimo_accum_frag)
                # --- Optional Z Gating + Down-Projection ---
                if reduceO:
                    lqk_PsiV_reshaped_frag = T.alloc_fragment([chunk_size, R, P], accum_dtype)
                    ...
                    if hasZ:
                        ...
                        z_expanded_frag = T.alloc_fragment([chunk_size, R, P], accum_dtype)
                        ...
                        o_gated_mul_tmp = T.alloc_shared([chunk_size, R, P], accum_dtype)
                        ...
                        for cs, r, p in T.Parallel(chunk_size, R, P):
                            lqk_PsiV_reshaped_frag[cs, r, p] *= phi_frag_intrachunk[0, r, p]
                ...
                # for csr, n in T.Parallel(fused_chunk_size, N): NOTE
                #     k_state_frag[csr, n] *= dA_cs_rev_frag[csr//R]
                for cs in T.serial(chunk_size):
                    T.vmul(k_state_frag[cs*R:cs*R+R, :], dA_cs_rev_frag[cs], k_state_frag[cs*R:cs*R+R, :])
                ...
                # for n, p in T.Parallel(N, P): NOTE
...
def mamba_mimo_forward(q, k, v, 
    ...
    h = torch.empty((B, H, N, P), device='npu', dtype=torch.float32) if return_state else None
    k_final = torch.empty((B, R, H, N), device='npu', dtype=dtype) if return_state else None
...
def run_test():
    ...
    # print("tilelang out:")
    # print(out_tilelang[1, :16, 1, :16])
    # print("ref out:")
    # print(out_ref_fp32[1, :16, 1, :16])
    print(f"tilelang nonzero: {torch.count_nonzero(out_tilelang)}")
    print(f"ref nonzero: {torch.count_nonzero(out_ref_fp32)}")
    compare("O", out_tilelang, out_ref_fp32)       
